...
object TheFiberModel_Solutions {
  ...
  object Question3 {
    val printNumbers =
      ZIO.randomWith(_.nextInt).repeat(Schedule.fixed(1.second))
  ...
  object Question5 {
    ...
    def run = for {
      ...
      _ <-
        ZIO.debug(
          "Operation Finished!"
        ) // <- will never reach here and operation will never stop
  ...
  object Question6 {
    ...
    val child =
      (Console.printLine("Child fiber 1 beginning execution...").orDie *>
        ZIO.sleep(5.seconds) *>
        Console.printLine("Hello from a child fiber 1!").orDie)
        .onInterrupt(ZIO.debug("Child fiber 1 is interrupted"))

    val child2 =
      (Console.printLine("Child fiber 2 beginning execution...").orDie *>
        ZIO.sleep(5.seconds) *>
        Console.printLine("Hello from a child fiber 2!").orDie)
        .onInterrupt(ZIO.debug("Child fiber 2 is interrupted"))
    ...
    val parent =
      (Console.printLine("Parent fiber beginning execution...").orDie *>
        ...
        .onInterrupt(ZIO.debug("Parent Fiber is interrupted"))
    ...
    def run =
      for {
        ...
        _           <- Console.printLine("---- Program complete ----").orDie
      } yield ()
