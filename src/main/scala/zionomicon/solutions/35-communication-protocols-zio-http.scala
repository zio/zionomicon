...
package CommunicationProtocolsZIOHTTP {
  ...
  package RequestDurationLogging {
    ...
    object Solution {
       ...
       * request.
       *
       * Uses HandlerAspect.interceptHandlerStateful to share state (start time)
       * between the incoming and outgoing pipelines. This allows us to:
       ...
       * immediately returns through the outgoing pipeline, and we still log the
       * correct duration for that failed request.
       */
      /**
       * Creates a request logging middleware. Logs when each request completes
       * with its response status.
       *
       * This middleware demonstrates the Middleware pattern in ZIO HTTP, which
       * allows intercepting and transforming request handlers. The middleware
       * uses handler.tapZIO to log response status after the handler completes.
       *
       ...
       * middleware would require using HandlerAspect with stateful composition
       * (tracking start times across async boundaries).
       *
          ...
          override def apply[Env1 <: Any, Err](
            ...
            routes.transform { handler =>
              handler.tapZIO { (response: Response) =>
                ZIO.debug(s"Request completed with status: ${response.status}")
              }
            }
        }
