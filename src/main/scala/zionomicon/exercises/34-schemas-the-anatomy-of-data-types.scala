...
package SchemaTheAnatomyOfDataTypes {
   ...
   *
   *      Hint: You will need to add new case classes to the `Query` sealed trait
   *      for each operator, and update the `FieldAccessor` class to provide these
   ...
   *
   *      Hint: Use ZIO Schema's ability to convert values to/from `DynamicValue`
   *      to implement this codec. You can follow the same pattern as the JSON
   *      codec shown in the chapter, using `schema.toDynamic` for encoding and
